...
> **⚠️（重要）この Step は `proj-default-swedencentral` プロジェクトで実施してください。** 他の Step で使っているプロジェクト（East US 2 リージョン）は **Microsoft 365 / Teams への公開が未サポート**で、公開ダイアログが「初期化に失敗しました」でエラーになります。公開対応リージョンの **`proj-default-swedencentral`（Sweden Central）** でエージェントを作成・公開してください。
>
> **プロジェクトの切り替え方**: ポータル左上の **「Microsoft Foundry /」の右にあるプロジェクト名をクリック**し、一覧から **`proj-default-swedencentral`** を選択します。
...
## 事前準備
- **この Step は `proj-default-swedencentral`（Sweden Central）プロジェクトで実施すること**（他リージョンは M365 / Teams 公開が未サポート）。ポータル左上の **「Microsoft Foundry /」の右のプロジェクト名をクリック**して `proj-default-swedencentral` に切り替える。
...
### E. 動作を確認する
1. **Just you** の場合: **Microsoft 365 Copilot（<https://m365.cloud.microsoft/chat/>）** を開き、エージェント ストアの **Your agents** に自分の Agent が表示されることを確認する。開いて質問し、応答を確認する。
