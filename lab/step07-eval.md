...
1. **データソース**で **既存のトレース** を選び、期間（日単位）でトレースを対象にする。
   - トレースはインジェストに 3〜5 分かかるため、**評価する時刻の 10 分以上前**のトレースを対象にする。
2. **フィールドのマッピング**で、各項目をデータセットの実列に割り当てる。
   - **クエリ**と**応答**は自動検出され `{{item.query}}` / `{{item.response}}` が入る。
...
3. **条件（評価器）** は、**自動提案された 20 個をそのまま使ってよい**（エージェント9＋品質4＋安全性7）。
   - ツール項目をマッピング済みなら、tool 系・agent 系も含めて実行できる。
   - ⚠️ が消えない場合は下の「回避策」を参照。
4. **完全な評価を実行する** をクリックし、**評価** タブで結果（各ケースのスコア・集計）を確認する。
