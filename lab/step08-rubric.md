...
- 採点用のジャッジ モデルとして **gpt-5.4**（または gpt-5.4-mini / gpt-5.5 など推奨モデル）のデプロイがあること。
  - **（事実）** Rubric のジャッジ推奨モデル: gpt-5.5 / gpt-5.4 / gpt-5.4-mini（コスト対性能のバランス最良）など。
...
2. 「エバリュエーターの作成」画面で、次のとおり入力する。
   ...
   | **対象エージェント（省略可能）** | Step 3〜5 の Agent `handson-agent-<自分の名前>` を選択 | 選ぶと、その Agent の instructions を文脈として Rubric を生成する。 |
   | **プロンプト** ✱ | **対象エージェントを選ぶと自動設定されるプロンプトのままでOK**（より良くしたい場合は下記の例文で上書き） | 対象エージェントを選ぶと、その Agent の instructions が自動でこの欄に入る。生成器はこれを手がかりに採点観点を推定するので、そのままでも妥当なルーブリックが作れる。評価観点を明示したい場合のみ下記例文に置き換える。必須。 |
   ...
   **プロンプト欄について**：対象エージェントを選ぶと、この欄には Agent の instructions（システムプロンプト）が自動で入ります。これは「エージェントがどう振る舞うか」の説明で、**そのまま生成しても妥当なルーブリックが作れます**。一方この欄は本来「**何を評価するか（採点の観点）**」を書く欄なので、狙った観点を出したいときは次の例文に**上書き**するとブレが減ります（任意）。
