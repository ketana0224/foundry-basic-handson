...
2. 「エバリュエーターの作成」画面で、次のとおり入力する。
   ...
   | **対象エージェント（省略可能）** | Step 3〜5 の Agent `handson-agent-<自分の名前>` を選択 | 選ぶと、その Agent の instructions が **エージェントの指示** 欄に自動で入り、それを文脈として Rubric を生成する。 |
   | **プロンプト** ✱ | **下記の例文を入力する** | **エージェントの指示 欄は自動で入るが、この プロンプト 欄は自動では入らない。** この欄は「何を評価するか（採点の観点）」を書く欄なので、下記の例文を貼り付ける。必須。 |
